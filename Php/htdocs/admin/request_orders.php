...
<body>
  ...
  <div class="col-sm-6">
    ...
    <a href="viewUser.php" class="w-100 btn btn-warning btn-sm">User</a>
    <a href="request_service.php" class="w-100 btn btn-success btn-sm">Service</a>
    <a href="request_orders.php" class="w-100 btn btn-warning btn-sm">Order</a>
    <a href="logout.php" class="w-100 btn btn-danger btn-sm">Sign out</a>

    <h3>Web Application Quotation : Request Orders</h3>
  ...
  <div class="container">
    ...
    <div class="col-sm-6">
      ...
      <input type="text" name="phone" id="" class="form-control" placeholder="เบอร์ติดต่อ"><br>
      <input type="text" name="email" id="" class="form-control" placeholder="E-mail"><br>


      <select class="form-control" name="jobsid_1" required>
        <option value="" >เลือกรับบริการ / รายละเอียด </option>
        <?php foreach ($query_jobs as $value) { ?>
          <option value="<?=$value['id']?>"><?=' | '.$value['jobs_name'].' | '.$value['details_th'].' | '.$value['prise'] .' บาท '?></option>
        <?php } ?>
      </select>
      <br>
      <select class="form-control" name="jobsid_2">
      <option value="" >เลือกรับบริการ / รายละเอียด </option>
        <?php foreach ($query_jobs as $value) { ?>
          <option value="<?=$value['id']?>"><?=' | '.$value['jobs_name'].' | '.$value['details_th'].' | '.$value['prise'] .' บาท '?></option> 
        <?php } ?>
      ...
      <br>
      <select class="form-control" name="jobsid_3" >
      <option value="" >เลือกรับบริการ / รายละเอียด</option>
        <?php foreach ($query_jobs as $value) { ?>
          <option value="<?=$value['id']?>"><?=' | '.$value['jobs_name'].' | '.$value['details_th'].' | '.$value['prise'] .' บาท '?></option>
        <?php } ?>
      ...
      <br>
      <select class="form-control" name="jobsid_4">
      <option value="" >เลือกรับบริการ / รายละเอียด</option>
        <?php foreach ($query_jobs as $value) { ?>
          <option value="<?=$value['id']?>"><?=' | '.$value['jobs_name'].' | '.$value['details_th'].' | '.$value['prise'] .' บาท '?></option>
        <?php } ?>
      ...
      <br>
      <select class="form-control" name="jobsid_5">
      <option value="">เลือกรับบริการ / รายละเอียด</option>
        <?php foreach ($query_jobs as $value) { ?>
          <option value="<?=$value['id']?>"><?=' | '.$value['jobs_name'].' | '.$value['details_th'].' | '.$value['prise'] .' บาท '?></option>
        <?php } ?>
      ...
      <br>
      <select class="form-control" name="jobsid_6">
      <option value="" >เลือกรับบริการ / รายละเอียด</option>
        <?php foreach ($query_jobs as $value) { ?>
          <option value="<?=$value['id']?>"><?=' | '.$value['jobs_name'].' | '.$value['details_th'].' | '.$value['prise'] .' บาท '?></option>
        <?php } ?>
    ...
    <div align="right"><a href="./vieworders.php"><button class="btn btn-outline-dark">View Order</button></a></div>
  </div>
</body>
